...
Per exemple imaginem que llancem dos daus. L'espai mostral serà $\Omega={(1,1),(1,2),(1,3),....(6,5),(6,6)}$ format per les 36 parelles de resultats possibles. Si els daus no estan trucats, cada parella té la mateixa probabilitat 1/36. Si es considera A={amb dues cares son senars}  el número de possibilitats son 9  i per tant la probabilitat de A es P(A)=9/36=1/4. 
...
### Conceptes bàsics: Variable aleatòria, funcións de distribució i probabilitat.
...
Quan es fa intervenir una variable aleatòria és per que s'està en la presència d'un espai de probabilitat. La variable aleatòria trasllada la informació d'aquesta probabilitat de $\Omega$  a $\mathbb{R}$   mitjanançant la probabilitat induïda coneguda com llei de probabilitat o distribució de probabilitat.
...
En cap cas la suma de les dues cares de un dau son 0, però ens 6 situacions les dues cares poden ser iguals. Si calculem amb els dues distribucións la probabilitat de que la variable aleatòria sigui 0, dona resultats diferents.
...
Les distribucións de probabilitat com la $P_X$, encara que ens presenta la informació necessària per obtenir les probabilitats, son objectes matemàtics complexes d'utilitzar i es recorre a dos tipus de funcións associades, la  Funció de distribució de probabilitat o la funció de probabilitat o densitat de probabilitat, depenen de la natura de la variable.
...
La funció de distribució té les següents propietats
...
Alguns exemples de variables aleatòries discretes que venen definides per les seves funcións de probabilitat son les  que tenen una distribució Poisson, Binomial, Hipergeomètrica ó binomial negativa. La seva definició es pot trobar en l'annex del tema.
...
:::{.example}
Alguns exemples de variables aleatòries continues que venen definides per les seves funcións de  densitat probabilitat son les  que tenen una distribució  uniforme, Normal, Exponencial, Gamma  entre altres. La seva definició es pot trobar en l'annex del tema.
:::
...
$$
És senzill comprobar que $P_X$ verifica els tres axiomes de probabilitat, pel que $(\mathbb{R}^k,\beta^K, P_X)$ permet indueix un espai de probabilitat amb les característiques de $(\Omega,\mathcal{A},P)$ heretats mitjanançant X.
...
En els apartats anteriors hem definit la probabilitat conjunta associada a una variable multidimensional. Com en el cas unidimensional, podem caracteritzar  aquesta probabilitat conjunta a partir de funcións conjuntes de distribució o de  densitat o de probabilitat segon siguin les variables.
...
En aquesta regla calculem la probabilitat d'un rectangle a partir de les funcións de distribució conjunta en cada punt del rectangle que es pot veure en la figura   \@ref(fig:img-rectangle)
...
Si tornem a l'exemple del llençament de la moneda i el dau de l'exemple \@ref(exm:quali1) podem definir la funció de probabilitat continua com el  producté de la probabilitat d'obtenir el resultat al llençar la moneda ( 1/2) per la probabilitat de obtenir un resultat del dau, es a dir (1/6). Així $P_XY(x_i,y_i)=1/2 \times 1/6= 1/12$
...
:::{.Noté }
_Nota: en els marges de la taula es poden veure les funcións de probabilitat marginals que seran definidas en la propera secció._ 
...
En la figura \@ref(fig:img-densconjunta) es pot observar la representació de la funció de densitat $f_{XY}(x,y)$ d'una variable aleatòria continua bivariada. La superfície de la gràfica es el valor de la funció la probabilitat ve determinada pel volum. Aquest volum ve calculat pel que s'anomena integral doble que té algunes especificitats que mostrarem en el següent apartat.  
...
Si integren primer en funció de y la integral interior és, tenint en compté que x es constant : 
...
Considerem un cas on el recinté d'integració no és un rectangle i utilitzem la funció de l'exemple \@ref(exm:densitat1) f(x,y)=x+y 
...
La integral doble en el recinté de la funció es pot calcular de dues maneres començant primer per $x$ o per $y$
...
obtenim que  el valor de la integral en el triangle es 12. En aquest cas hem fixat $x$ i hem trobat el rang de variació de $y$ dins del recinté d'integració A en funció de $x$ com es pot veure en la figura  \@ref(fig:graf-triangle) esquerra.
...
Les distribucións marginals son les distribucións unidimensionals que ens informa de la distribució dels valors d'una variable prescindint dels valors de les altres. 
...
Cal tenir en compté que per una component 
...
El concepté de marginalitat pot aplicar-se a qualsevol subvector del vector original . Així per al subector $X^l=(X_{i_1},...,X_{i_l})$ on $l \leq k$  podem parlar de la distribució conjunta marginal de $X^l$ a la expressió on es fixan les components del subvector i es fan tendir a l'infinit a la resta 
...
Generem un triangle  T amb vèrtexs en les coordenades (0,0), (1,0) (0,2),  como es veu en la figura \@ref(fig:graf-triangle2). Repartim la masa de probabilitat uniformment en el triangle donat que l'elegim a l'atzar un punt del triangle.  Per a un àrea qualsevol  la probabilitat de aquella àrea es
...
Así podem obtenir la funció conjunta simplement aplicant una mica de geometria segons l'ubicació del punt (x,y) en el pla aplicant la definició de la funció de distribució conjunta con la intersecció amb el triangle que l'equació de la hipotenusa es 2x+y=2 
...
- Si $x\leq 0$ per qualesvol y  $F_x(x,y)=0$ i per tant el limit quan $\lim_{y \rightarrow +\infty}$ es 0                      
...
Analogament podem obtenir la  funció de distribució de $Y$  on la diferència es que  entre 0 i 2 quan $x \rightarrow+\infty$ estarem en punts tipus 4 i  la funció valdrà $y-\frac {y^2}{4}$
...
Es pot demostrar que tant $F_X(x)$ com $F_Y(y)$ son funcións de distribució  . En el següent exemple 
...
:::{.example}
 ...
 En la figura \@ref(fig:marginal) es mostra una representació de la distribució marginal. El diagrama de dispersió representa les coordenades $(x,y)$ de les observacions. Si es projecten en l'eix $Y$ totes les dades sense tenir en compté la component $X$, s'obté la función de distribució marginal de $Y$.
...
$f_{X|Y}(x|y)$ es efectivamkenté una función de probabilitat per qué complexis les dues propietats
...
Això serà rellevant més endavant quan parlem del concepté d'independència 
:::  
...
Siguin X i Y dues variables aleatòries continues on $f_{XY}$ es la funció de densitat conjunta , i $f_X$ i $f_Y$ son les funcións de densitat marginal.  Definim la densitat condicional de X donat que $Y=y$ a la funció 
...
$$
que sería la mitjanana poblacional de la distribució de la variable $X$
...
$$
que sería la mitjanana poblacional de la distribució de la variabla $X$
:::
...
\hline
\text{Respecté del origen} & X^k & |X|^k \\
\hline
\text{Respecté de } a & (X-a)^k & |X-a|^k \\
\hline
...
$$
   ...
   Així el moment d'ordre 1 es  la <b>$E[X]=\mu$ mitjanana</b> de la distribució  i el moment central d'ordre 2 es la <b>variància $Var[X]=\sigma_X^2=E[(X-\mu)^2]= E[X^2]-(E[X])^2$.</b> 
...
Una barra d’un metre es trenca aleatòriament en dos punts, generant tres peces. Quina és la llargada mitjanana de la peça del mig?
...
$$ 
que dona lloc a $E[X_1^{n_1} .....X_k^{n_k}]$ respecté a l'origen de cada component. 
...
El moment conjunt central d'ordre k respecté de l'origen de cada component es un cas particular , que es pot obtenir sempre que l'esperança existixca
...
$$
Així mateix si $g(X)=\sum_{i=1}^k (a_i X_i)$ obtenim que l'esperançade d'una constant per una variable es la constant per l'esperança de la variable
...
$$
En general.  Si $X=(X_1,....X_k)$ és un vector aleatori de dimensió k la matriu de variancies-covariàncies és
...
2. el  Jacobiano de g, $J=\frac{\partial (g_1,....,g_k)}{\partial (x_1,...,x_k) }$ es diferenté de cero $\forall x \in D_X$
...
<small>
* Nota:  El concepté de independència es veura en el tema següent
</small>
...
### Producté de dues variables aleatòries

En el cas del producté W=XY i com abans definint V=X tenim que podem calcular la distribució conjunta (W,V) aplicant el teorema  \@ref(thm:thm-cambio)  partir de les funcións inverses  X=V i Y=W/V i el Jacobià 
...
I la distribució marginal del producto s'obté integran respecté a l'altra component
...
Volem obtenir la densitat del producté $W=XY$ que seguint l'apartat anterior es pot obtenir  comn
...
Per tant, la densitat del producté és
...
En el cas del quocient $Q=XY$ i com abans definint V=X tenim que podem calcular la distribució conjunta (Q,V) aplicant el teorema  \@ref(thm:thm-cambio)  partir de les funcións inverses  X=V i Y=QV i el Jacobià 
