...
A qualsevol que se li pregunti quin temps es triga a recórrer els 280 quilòmetres de distància entre Barcelona i Castelló, si es circula a una velocitat constant de 120 km/h,  ens indicaria que en 2 hores i 20 minuts arribaríem a la destinació. Ara bé, si se li pregunta quin serà el resultatt de  llançar un dau  no tindria una resposta clara.
  
  * El primer resultatt prové d'un fenomen o **experiència determinista.** N'hi ha prou amb extreure de l'equació  $e=v·t$  el valor del temps
  * El segon pertany a la categoria dels fenòmens o **experiències aleatòries.** Es coneixen tots els resultatts possibles, un número del 1 al 6 en el cas del dau, però no quin resultatt es produirà, ja que de repetició en repetició sortiran resultatts diferents.
...
Un **experiment** és una seqüència d’accions que, en repetir-se sota les mateixes condicions, produeix resultatts observables.  Direm que l'experiment es **aleatori** si, executat sota les mateixes condicions, pot donar lloc a un resultatt $w$ d'entre un conjunt de posibles resultatts.
...
L’**espai mostral** d’un experiment, denotat per $\Omega$, és el conjunt de tots els resultatts possibles.
:::


:::{.definition}
Un **esdeveniment**  es una col·lecció  de possibles resultatts d'un experiment, es a dir un subconjunt de $A \subseteq \Omega$.  Quan el resultatt de l'experiment pertany al esdeveniment ($w \in A$), direm que a ocorregut l'esdeveniment. 
:::
...
Per exemple imaginem que llancem dos daus. L'espai mostral serà $\Omega={(1,1),(1,2),(1,3),....(6,5),(6,6)}$ format per les 36 parelles de resultatts possibles. Si els daus no estan trucats, cada parella te la mateixa probabilitat 1/36. Si es considera A={amb dues cares son senars}  el número de possibilitats son 9  i per tant la probabilitat de A es P(A)=9/36=1/4. 
...
Si tornem a l'exemple de llençar dos daus podem definir dues variables aleatòries. *X* que sigui la suma dels resultatts dels dos daus i *Y* que sigui la diferència dels resultatts. Encara que les dues variables estàn definides en el mateix espai de probabilitat, $P_X$ i $P_Y$ son diferents per que estan induïdes per variables diferents.

En cap cas la suma de les dues cares de un dau son 0, però ens 6 situacions les dues cares poden ser iguals. Si calculem amb els dues distribucions la probabilitat de que la variable aleatòria sigui 0, dona resultatts diferents.
...
Si tornem a l'exemple del llençament de la moneda i el dau de l'exemple \@ref(exm:quali1) podem definir la funció de probabilitat continua com el  producte de la probabilitat d'obtenir el resultatt al llençar la moneda ( 1/2) per la probabilitat de obtenir un resultatt del dau, es a dir (1/6). Així $P_XY(x_i,y_i)=1/2 \times 1/6= 1/12$
...
$$
Començant primer per fixar la $y$ , hem de avaluar el rang de variació de $x$ i obtenim el mateix resultatt al efectuar l'integració( Figura \@ref(fig:graf-triangle) dreta) Per lo que done igual per on començar  a l'hora de fer l'integral doble. S'obtenen els mateixos resultatts. 
...
Si  revisem la taula del exemple  \@ref(exm:taula-discreta), on en primer lloc es llança una moneda i  després un dau. Observem que la darrera fila conté les probabilitats marginals d'observar cara o creu, obteneint un valor de 1/2 per cada columna. La darrera columna conté les marginals d'obtenir un resultatt de les cares del dau , és a dir 1/6 per cada fila.
...
Tornem a  l'exemple de la moneda  i el dau  i la taula de probabiliatats que es mostra en l'exemple \@ref(exm:marginal) on Y es el valor del dau i  X es el resultatt de la moneda. 

Calculem la probabilitat de treure una cara donat que el dau ha resultatt el valor 4. 
...
Que es la coneguda distribució triangular resultatt de sumar dues variables independents* U(0,1)
