...
Tornem a  l'exemple de la moneda  i el dau  i la taula de probabiliatats qe es mostra en l'exemple \@ref(exm:marginal) on Y es el valor del dau y X es el resultat de la moneda. 
...
\begin{cases}
 4 y^3 \int_0^{1/y} xdx= y^3 \left [\frac {x^2}{2}\right]_0^{1/y} 
 =2y ,              & \text{si } y \in (0,1) \\
0,              & \text{en la resta }  \\
\end{cases}
$$
...
$$
   ...
   Així el moment d'ordre 1 es  la **$E[X]=\mu$ mitjana** de la distribució  i el moment central d'ordre 2 es la **variancia $Var[X]=\sigma_X^2=E[(X-\mu)^2]= E[X^2]-(E[X])^2$.** 
...
**Esperanza d'un vector alatori discret**
...
**Esperanza d'un vector alatori continu**
 ...
 $$
:::
En ambos caos l'esperança esta supeditada a que |g(x)f(x)| siguin absolutamente sumplae o integrable.
