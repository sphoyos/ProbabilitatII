...
X és una variable aleatòria discreta si  $X(\Omega)$ es  finit o numerable, és a dir es pot definir un conjunt $D_X=\{x_1,x_2,....,x_n \}$ tal que $P_X(D_X))P(X \in D_X)=1)$ i per tant es pot deduir que $P(X=x)=0 \quad, \forall x \in D^zc_X$
...
**Algunes propietats de la funció de  o probabilitat**
...
on $\parallel A \parallel$ és l'àreade B.  
...
Sigui $D_{X_i}$  el soport numerable de la component $X_i$ d'un vector aleatori $X$ discret, es verifica que 
...
Si  revisem la taula del \@ref(exm:taula-discreta) exemple on en primer lloc es llança una moneda , o després un dau, observem que la darrera fila conté les probabilitats marginasl d'observar cara o creu que son 1/2 i la darrera columna conté les marginasl d'obtenir un resultat de les cares del dau , és a dir 1/6 per cada valor.
:::
...
Si les dues variables $X$ i $Y$ con continues, com hem vist abans, perdefinició la distribució marginal de cadascuna de elles la podem escriure com:
