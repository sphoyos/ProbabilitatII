...
Suposem un espai mostral $\Omega$, numerable i com a $\sigma~àlgebra$  la formada per tots els subconjunts de $\Omega$. Definim un funció no negativa sobre tots els subconjunts de $\Omega$ on es verifica que $\sum_{w\in \Omega}p(w)=1$. Si es defineix  $P(A)=\sum_{w\in A}p(w)$, es pot comprovar que P es una probabilitat.
