...
\section{Variables aleatòries multivariades}\label{variables-aleatuxf2ries-multivariades}
...
\tightlist
\item
  El primer resultat prové d'un fenomen o \textbf{experiència determinista.} N'hi ha prou amb extreure de l'equació \(e=v·t\) el valor del temps
\item
...
\begin{definition}
Un \textbf{experiment} és una seqüència d'accions que, en repetir-se sota les mateixes condicions, produeix resultats observables. Direm que l'experiment és \textbf{aleatori} si, executat sota les mateixes condicions, pot donar lloc a un resultat \(w\) d'entre un conjunt de posibles resultats.
\end{definition}
...
\begin{definition}
Un \textbf{esdeveniment} es una col·lecció de possibles resultats d'un experiment, és a dir un subconjunt de \(A \subseteq \Omega\). Quan el resultat de l'experiment pertany al esdeveniment (\(w \in A\)), direm que a ocorregut l'esdeveniment.
\end{definition}
...
Una col·lecció \(\mathcal{A}\subseteq \mathcal{P}(\Omega)\) és una \textbf{\(\sigma\text{-àlgebra}\)} si:
...
En resum, L'espai mostral ve acompanyat de la \(\sigma-álgebra\) mes convenient a l'experiment. La combinació (\(\Omega,\mathcal{A}\)) rep el nom d'\textbf{espai mesurable}
...
A la \emph{terna \((\Omega,\mathcal{A},Pr)\)} l'anomenarem \textbf{espai de probabilitat}.
\end{definition}
...
Si considerem dos espais de probabilitat \((\Omega, \mathcal{A})\) i \((\mathbb{R},\beta)\), una \textbf{variable aleatòria} es una aplicació \(X:\Omega \rightarrow \mathbb{R}\) que verifica que \(X^{-1}(B) \in \mathcal{A}\) \(\forall B \in \beta\)
\end{definition}
...
\begin{equation}
F_X(x)= P_X((-\infty,x])=P(X^{-1}\{(-\infty,x]\})=P(X\leq x) \quad \forall x \in \mathbb{R} \label{eq:Fx}
\end{equation}
\end{definition}
...
\item
  \(\lim_{x \rightarrow \infty} F_X(x)=1\) i \(\lim_{x \rightarrow -\infty} F_X(x)=0\)
\end{enumerate}
...
\begin{equation}
 F_x(x)=\sum_{x \leq x_i} P(X=x_i) \label{eq:Fxdiscret}
\end{equation}
...
\begin{definition}
\textbf{Funció de densitat o probabilitat d'una variable aleatòria continua}
...
\[
P(X \in ]a,b]= P(a<X \leq b) \int_{a}^{b}  f(x) dx \label{eq:pxcontint}
\]
...
\[
P_X(B)=P(\mathbf{X}^{-1}(B)), \quad \forall B \in \beta^K \label{eq:pvectorind}
\]
...
\begin{definition}
\textbf{Variable aleatòria bidimensional o bivariant}
...
\[
P_{(X,Y)}(B)=P((X,Y)^{-1}(B))=P(\{w \in \Omega:(X(w),Y(w)) \in B\})  \label{eq:pconjunt}
\]
Aquesta mesura de probabilitat induïda s'anomena \textbf{distribució de probabilitat conjunta de (X,Y)}
...
\begin{definition}
\textbf{Variable aleatòria k- dimensional o multivariant}
...
\[
F_X( \mathbf{x} )=F_X(x_1,...,x_k) =P_X(S_x)=P(X \in S_x)= P(\bigcap_{i=1}^{k} \{ X_i\leq x_i \}) \label{eq:Fdistconjunt}
\]
\end{definition}
...
\emph{Nota: en els marges de la taula es poden veure les funcions de probabilitat marginals que seran definides en la propera secció.}
...
\subsubsection{Variables aleatòries multivariants continues}\label{variables-aleatuxf2ries-multivariants-continues}

\begin{definition}
\textbf{Funció de densitat conjunta de probabilitat d'una variable aleatòria multivariat absolutament continua}

Sigui \(\overrightarrow{\bf{X}}=(X_1,X_2,...,X_{k-1},X_k)\) un vector aleatori on \(X_i\) es una variable aleatòria continua. Diguem que \textbf{X} és continu si i només si existeis una funció \(f_X:   \mathbb{R}^k \rightarrow[0,+\infty[\) tal que
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:graf-triangle}Dues maneres d'obtenir la densitat conjunta d'un triangle}]{Probabilitat-II_files/images/graf-triangle-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:density2}Funció de densitat f(x,y)}]{Probabilitat-II_files/images/density2-1.pdf}}
...
\[
F_{X^l}(X_{i_1},...,X_{i_l})=\lim_{x_j \xrightarrow {j \neq i_1,...,i_l} +\infty} F_X(x_1,...x_k)  \label{eq:Fmarginal}
\]
\end{definition}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:graf-triangle2}Dues maneres d'obtenir la densitat conjunta d'un triangle}]{Probabilitat-II_files/images/graf-triangle2-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:marginal}Funció de distribució marginal de Y mostral}]{Probabilitat-II_files/images/marginal-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:condicionada}Funció de distribució condicionada de Y en funció de X mostral}]{Probabilitat-II_files/images/condicionada-1.pdf}}
...
\subsubsection{Moments univariants: Esperança i Variància}\label{moments-univariants-esperanuxe7a-i-variuxe0ncia}
...
\textbf{Esperança d'una variable aleatòria discreta}
...
\textbf{Esperança d'una variable aleatòria continua}
...
\subsubsection{Moments multivariants: Esperança i Variància}\label{moments-multivariants-esperanuxe7a-i-variuxe0ncia}

\textbf{Esperança d'un vector aleatori discret}
...
\textbf{Esperança d'un vector aleatori continu}
...
\tightlist
\item
  La covariància és un operador simètric: \(Cov(X, Y ) = Cov(Y,X)\)
\item
...
\[
Cov(aX, Y ) = Cov(X, aY ) = aCov(X, Y )    \label{eq:provcov1}
\]
\[
Cov(X + Y,Z) = Cov(X,Z) + Cov(Y,Z)   \label{eq:provcov2}
\]
\[
Cov(X, Y + Z) = Cov(X, Y ) + Cov(X,Z)   \label{eq:provcov3}
\]

\[
\text{5. }  Var(X + Y ) = Var(X) + Var(Y ) + 2Cov(X, Y )   \label{eq:provcov4}
\]
\[
\text{6. } Var(X − Y ) = Var(X) + Var(Y ) − 2Cov(X, Y )   \label{eq:provcov5}
\]
...
\begin{theorem}
\protect\hypertarget{thm:thm-cambio}{}\label{thm:thm-cambio}Sigui \(X=(X_1,..., X_k)\) un vector aleatori continu amb soport \(D_X\), és a dir \(P((X_1,...,X_k) \in D_X)=1\), i sigui \(g=(g_1,...,g_k):\mathbb{R^k} \rightarrow  \mathbb{R^k}\) una funció vectorial que verifica:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  g es unívoca sobre Dx
\item
...
\begin{example}
\protect\hypertarget{exm:circulo}{}\label{exm:circulo}Sigui l'experiment aleatori triar un punt a l'atzar del cercle unitat \(C_1\), on podem definir sobre l'espai de probabilitat un vector aleatori de les coordenades del punt (X,Y). L'elecció a l'atzar implica una probabilitat uniforme sobre \(C_1\). La densitat conjunta la podem expressar com
...
\subsubsection{Suma de dues variables aleatòries}\label{suma-de-dues-variables-aleatuxf2ries}
...
\textbf{Cas (X,Y) variable aleatòria bivariant discreta}
...
\textbf{Cas (X,Y) variable aleatòria bivariant continua}
...
\begin{example}
\protect\hypertarget{exm:suma}{}\label{exm:suma}Siguin X i Y dues variables aleatòries amb distribució uniforme al interval (0,1) i amb densitat conjunta
...
\subsubsection{Producte de dues variables aleatòries}\label{producte-de-dues-variables-aleatuxf2ries}
...
\subsubsection{Quocient de dues variables aleatòries}\label{quocient-de-dues-variables-aleatuxf2ries}
...
\section{Independència de variables aleatòries}\label{independuxe8ncia-de-variables-aleatuxf2ries}
...
\begin{definition}
\protect\hypertarget{def:indep}{}\label{def:indep}\textbf{Variables aleatòries independents}
...
Comprovar la independència de variables aleatòries a partir de la equació \eqref{eq:pxindep} es complicat i es mès fàcil caracteritzar la independènciaa partir de les funcions de distribució i densitat conjuntes i marginals.

\begin{theorem}
\textbf{Teorema de factorització}
...
\[ 
1.\qquad F_X(x_1,...,x_k)= \prod_{l=1}^k F_j(x_j), \quad \forall (x_1,...,x_k) \in \mathbb{R}^k \label{eq:Fxindep}
\]

\[
2. \qquad f_X(x_1,...,x_k)= \prod_{l=1}^k f_j(x_j), \quad \forall (x_1,...,x_k) \in \mathbb{R}^k \label{eq:fpxindep}
\]
\end{theorem}

\textbf{Demostració}

Suposem que les variables aleatòries contínues (X\_1,\ldots,X\_k) són independents. Per definició,
...
\begin{proposition}
\textbf{Independència i Esperança}

Si \(X_1,...,X_k\) són independents, aleshores \(E[X_1,...,X_k]=E[\prod_{j=1}^{k}X_i]=\prod_{j=1}^{k}E[X_i]\) \label{eq:indep}
...
\[
E[\prod_{j=1}^{k}g(X_i)]=\prod_{j=1}^{k}E[g_i(X_i)]\label{eq:gindep}
\]
:::\{.corollary\}
...
\subsection{Esperances i variàncies condicionades.}\label{esperances-i-variuxe0ncies-condicionades.}
...
=
\int_{\Omega} g(y)\, dP_{Y\mid X=x}(y).  \label{eq:espcond}
\]
\end{definition}

La forma de l'equació \eqref{eq:espcond} depen de les característiques de la distribució del vector (X,Y)
...
\begin{definition}
\textbf{Funció de esperança condicionada o regressió}
...
\tightlist
\item
  L'esperança condicionada es un operador lineal
  \[
  E[(a g_1(Y)+b g_2(Y))\mid X]=aE[g_1(Y)\mid X]+bE[g_2(Y)\mid X] \label{eq:expcondlineal} 
  \]
  En particular,
\end{enumerate}
...
\item
  \[
  P(a \leq Y \leq b) = 1 \implies a \leq E(Y|X) \leq b \label{eq:expcondinterval}
  \]
\item
  \[
  P(g_1(Y) \leq g_2(Y)) = 1 \implies E[g_1(Y)|X] \leq E[g_2(Y)|X] \label{eq:expconddesigual}
  \]
\item
  \(E[c|X]=c\) para c constant. \label{eq:expcondconstant}
\end{enumerate}

Però hi ha algunes propietats que son específiques de de l'esperança condicional

\textbf{Llei de l'esperança total}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Anar a reparar la màquina que s'espatlli i romandre al costat d'ella fins que un altra s'espatlle i anar llavors a reparar-la
\item
...
Si \(A_k\) es l'esdeveniment l'operari està en la màquina k. Per obtenir \(E_1[X]\) recorrem a la propietat de l'esperança total, però utilitzan com a distribució condicionada la que es deriva condicionar respecte de l'esdeveniment\(A_k\). Tindrem E\_1{[}X{]}=E{[}E(A\_k{]}{]}
...
\[
var(Y)=E(var[Y|X])+ var(E[X|Y])  \label{eq:varcond}
\]
\end{proposition}
...
\textbf{(a) Densitat marginal de \(X\)}
...
\textbf{(b) Densitat marginal de \(Y\)}
...
\textbf{(c) Densitat condicional de \((X\mid Y=y\)}
...
\textbf{(d) Densitat condicional de \((Y\mid X=x)\)}
...
\textbf{(e) Esperança i variància condicionades de \(X\mid Y=y\)}
...
\textbf{(f) Esperança i variància condicionades de \((Y\mid X=x)\)}
...
Amb aquests càlculs podem trobar la variable aleatòria Var(Y\textbar X) com
...
\subsubsection{Desigualtats}\label{desigualtats}
...
\[
 P(X \geq \epsilon) \leq \dfrac{E[X]}{\epsilon} \label{eq:desigepsilon}
 \]
...
\textbf{Desigualtat de Markov}

Si en la desigualtat \eqref{eq:desigepsilon} substituïm \(X\) per \(|X|^k\) i \(\epsilon\) per \(\epsilon^k\) tenim

\[
 P(|X| \geq \epsilon)=P(|X|^k \geq \epsilon^k) \leq \dfrac{E[|X|^k]}{\epsilon^k}    \label{eq:desmarkov}
 \]
coneguda com a desigualtat de Markov.

\textbf{Desigualtat de Txebixev}

Un cas especial de \eqref{eq:desmarkov}, que es coneix com la desigualtat de Txebixev, és el cas en el que k=2 i X=X-E{[}X{]}

\[
 P(|X-E[X]| \geq \epsilon) \leq \frac {1}{\epsilon^2}Var(X)     \label{eq:destxebixev}
 \]
...
\[
 E[g(X)] \geq g(E[X])   \label{eq:desjensen}
 \]
:::\{.Theorem \#thm-cauchy\}
\textbf{Desigualtat de Cauchy-Schwarz}

Si \(X\) i \(Y\) son variables aleatòries amb variàncies finites. Aleshores, \(cov(X,Y)\) existeix i es verifica que

\[
(E[XY])^2 \leq E[X^2]E[Y^2]  \label{eq:descauchy}
\]
...
\subsubsection{Covariància i Correlació}\label{covariuxe0ncia-i-correlaciuxf3}
...
A partir de la desigualtat de Cauchy-Schwartz \eqref{eq:descauchy} podem deduir que
...
\textbf{\emph{Matriu de Correlació}}
...
\subsubsection{Causalitat i correlació.}\label{causalitat-i-correlaciuxf3.}
...
\caption{Correlació espúria entre el consum de gelats i morts per ofegament en aigua en USA}\label{fig:img-icecream}
\end{figure}

En la URL \url{https://tylervigen.com/spurious-correlations} es poden trobar nombrosoes correlacions espúries on estàn disponibles les dades . D'ací la famosa frase en estadística \textbf{Correlation is not causation}, la correlació no implica causalitat.
...
\subsubsection{Distribució normal bivariant}\label{distribuciuxf3-normal-bivariant}

\begin{definition}
\textbf{Vector aleatori normal bivariant}
...
\[
f_{XY}(x,y)= \dfrac{1}{ 2 \pi \sigma_x \sigma_y \sqrt{(1-\rho^2)}} e^{-\dfrac{q(x,y)}{2}} , (x,y) \in  \mathbb{R}^2 \label{eq:fxybinorm}
\]
on

\[
q(x,y)=\dfrac{1}{1-\rho^2} \left\{ \left(\dfrac{x-\mu_x}{\sigma_x}\right)^2 -2 \rho \left ( \dfrac{x-\mu_x}{\sigma_x} \right) \left(\dfrac{y-\mu_y}{\sigma_y}\right)+ \left ( \dfrac{y-\mu_y}{\sigma_y} \right)^2   \right\} \label{eq:qxybinorm}
\]
\end{definition}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:graf-binormalrho}Densitat i corbes de nivell binormal amb \(\mu_x=\mu_y=0, \sigma_x=\sigma_y=1\) i \(\rho=0.8\)}]{Probabilitat-II_files/images/graf-binormalrho-1.pdf}}
\caption{\label{fig:graf-binormalrho}Densitat i corbes de nivell binormal amb \(\mu_x=\mu_y=0, \sigma_x=\sigma_y=1\) i \(\rho=0.8\)}
\end{figure}
...
\[
\Sigma= \begin{bmatrix} \sigma_x^2 & \sigma_{xy} \\\sigma_{xy}  & \sigma_y^2 \end{bmatrix}  \label{eq:sigmamat}
\]

\[
f(x,y)=\dfrac {|\Sigma|^{-\frac{1}{2}}}{2\pi} e^{ -\frac{1}{2} (\mathbf{x} - \boldsymbol{\mu})' \Sigma^{-1} (\mathbf{x} - \boldsymbol{\mu})} , \quad  (x,y) \in   \mathbb{R}^2  \label{eq:fxybinormmat}
\]
...
\[
|\Sigma|= \sigma_x^2 \sigma_y^2 - \sigma_{xy}^2=\sigma_x^2 \sigma_y^2(1-\rho_{xy}^2)  \label{eq:detsigma}
\]
...
\[
\begin{pmatrix} X \\ Y \end{pmatrix} \sim N \left( \boldsymbol{\mu} = \begin{pmatrix} \mu_x \\ \mu_y \end{pmatrix}, \Sigma = \begin{pmatrix} \sigma_x^2 & \rho\sigma_x\sigma_y \\ \rho\sigma_x\sigma_y & \sigma_y^2 \end{pmatrix} \right)   \label{eq:distbinorm}
\]
Queda per al lector la comprovació de l'equivalència de les expressions de les funcions de densitat.

\begin{definition}
\textbf{Distribució normal bivariant estàndard amb marginal independents}
...
\dfrac {1}{\sqrt{2\pi }}e^{\frac{-z_1^2}{2}} \dfrac {1}{\sqrt{2\pi }}e^{\frac{-z_2^2}{2}}= \dfrac {1}{2\pi }e^{(-\frac{1}{2})(z_1^2+z_2^2)}=-\dfrac {1}{2\pi }e^{(-\frac{1}{2})(\mathbf{z^{\prime}}\mathbf{z})}  \label{eq:fxybinormstand}
\]
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:graf-binormalrho0}Densitat i corbes de nivell binormal amb \(\mu_x=\mu_y=0, \sigma_x=\sigma_y=1\) i \(\rho=0\)}]{Probabilitat-II_files/images/graf-binormalrho0-1.pdf}}
\caption{\label{fig:graf-binormalrho0}Densitat i corbes de nivell binormal amb \(\mu_x=\mu_y=0, \sigma_x=\sigma_y=1\) i \(\rho=0\)}
\end{figure}

\begin{definition}
\textbf{Distribució normal bivariant estàndard amb marginals correlades }
...
\begin{proposition}
\textbf{Transformació d'una normal bivariant estándar en una normal bivariant general}
...
\subsubsection{Distribucions marginals i condicional de una normal bivariant.}\label{distribucions-marginals-i-condicional-de-una-normal-bivariant.}

\textbf{Distribucions marginals}
...
\caption{Marginals de la distribució continua bivariant}\label{fig:img-distmargi}
\end{figure}

\begin{proposition}
\textbf{Independència i correlació en una distribució normal bivariant}
...
\small
f_{X,Y}(x,y)=
\frac{1}{2\pi\sigma_x\sigma_y\sqrt{1-\rho^2}}\exp\left(-\frac{1}{2(1-\rho^2)}\left[\frac{(x-\mu_x)^2}{\sigma_x^2}-2\rho\frac{(x-\mu_x)(y-\mu_y)}{\sigma_x\sigma_y}+\frac{(y-\mu_y)^2}{\sigma_y^2}\right]\right)   \label{eq:fxybinormlarge}
\]
...
\begin{theorem}
\textbf{Distribucions condicionals}

Sigui \((X,Y)\) un vector aleatori que sequeix una distribució normal bivariant amb paràmetres \(\mu_x,\mu_y\in\mathbb{R}\) y \(\sigma_x,\sigma_y>0\) i \(\rho \in [-1,1]\) amb la funció de densitat definida en \eqref{eq:fxybinorm} corresponent a una distribució normal bivariant. La distribució condicional de \(y\) donat que \(X=x\) es una distribució normal amb la esperança i variància següent:

\[
E(Y|X=x]= \mu_Y+\rho \sigma_Y \left (  \dfrac {X-\mu_X}{\sigma_X}\right )   \label{eq:espcondbinorm}
\]

\[
Var(Y|X=x)=(1-\rho^2) \sigma_Y^2    \label{eq:varcondbinorm}
\]
\end{theorem}
...
\subsubsection{Distribució normal multivariant}\label{distribuciuxf3-normal-multivariant}

\begin{definition}
\textbf{Distribució normal multivariant}
...
\[
f(x_1,...,x_n=\dfrac {|\Sigma|^{-\frac{1}{2}}}{(2\pi)^\frac{1}{2}} e^{ -\frac{1}{2} (\mathbf{x} - \boldsymbol{\mu})' \Sigma^{-1} (\mathbf{x} - \boldsymbol{\mu})} , \quad  (x_1,...,,xn) \in   \mathbb{R}^n  \label{eq:fxybinormmulti}
\]
\end{definition}

Si les components del vector son independents, les covariàncies son totes nul·les i \(\Sigma\) es una matriu diagonal amb elements les variàncies de cada component . Així \(|\Sigma|^{-1}|=\dfrac{1}{\sigma_1^2\sigma_2^2...\sigma_n^2}\) A més la forma quadràtica de la equació \eqref{eq:fxybinormmulti} es simplifica i la densitat és:
...
\subsection{Distribucions especials: Multinomial, t-Student multivariant,Khi-quadrat, Wishart i Dirichlet.}\label{distribucions-especials-multinomial-t-student-multivariantkhi-quadrat-wishart-i-dirichlet.}

\subsubsection{Distribució Multinomial}\label{distribuciuxf3-multinomial}
...
\begin{example}
\protect\hypertarget{exm:multinomial}{}\label{exm:multinomial}Es coneix que hi ha quatre tipus de sang humana, O, A, B i AB. Es selecciona una mostra de persones donants de sang al atzar i podem estar interessats en conèixer la probabilitat de observar un número determinat de cada tipus de sang.
\end{example}
...
\[
= \begin{cases} \binom{n}{x_1, \dots, x_k} p_1^{x_1} \dots p_k^{x_k} & \text{if } x_1 + \dots + x_k = n, \\ 0 & \text{en la resta} \end{cases} \label{eq:fxymultinomial}
\]
\end{definition}
...
\begin{corollary}
\protect\hypertarget{cor:binomial}{}\label{cor:binomial}Si \(\mathbf{X}=(X_1,...,X_k)\) es un vector aleatori amb paràmetres n i \(\mathbf{p}=(p_1,...,p_k)\) . La distribució marginal de cada variable \(X_i,i=1,...,k)\) segueix una distribució binomial amb paràmetre \(n\) i \(p_i\)
\end{corollary}

\begin{corollary}
\protect\hypertarget{cor:agregacio}{}\label{cor:agregacio}Si \(\mathbf{X}=(X_1,...,X_k)\) es un vector aleatori amb parametres \(n\) i \(\mathbf{p}=(p_1,...,p_k)\) amb \(k \geq 2\) .Si \(\ell<k\) i \(i_1,...,i_{\ell}\) un subjconjunt de elements de \((1,...,k)\) La distribució de \(Y=Y_1+...+Y_k\) segueix una distribución binomial amb paramètre \(n\) i \(p_{i_1}+  ....+p_{i_\ell}\)
\end{corollary}

\begin{theorem}
\textbf{Esperances, Variàncies i covariàncies}
...
\]
Per altra banda de la equació \eqref{eq:provcov4} sabem que
\[
\begin{aligned} 
...
\subsubsection{Distribució t-student multivariant}\label{distribuciuxf3-t-student-multivariant}
...
\begin{definition}
\textbf{Distribució t-student multivariant}
...
\subsubsection{Distribució khi-quadrat}\label{distribuciuxf3-khi-quadrat}
...
\[
Y=\mathbf{X}^T \mathbf{X}= \sum_{i=1}^k  X_i^2 \sim \chi^2(k) \qquad  0>Q<\infty  \label{eq:chi2}
\]
\end{definition}

\begin{theorem}
La funció de densitat de la variable definida en \eqref{eq:chi2} ve definida per

\[
f_Y(y) = \frac{1}{2^{k/2} \Gamma(k/2)} y^{k/2 - 1} \exp[-y/2]    \label{eq:denschi2}
\]
\end{theorem}
...
\emph{1. Cas Base (\(k=1\)): Densitat de \(Y = X_1^2\)}
...
Aquesta expressió coincideix exactament amb la definició d'una distribució \textbf{Gamma} amb paràmetre de forma \(\alpha = 1/2\) i paràmetre d'escala \(\beta = 1/2\). Per tant:
...
\subsubsection{Distribució de Whishart}\label{distribuciuxf3-de-whishart}
...
On n\textgreater p-1 i V es una matriu simètrica definida positiva \(p \times p\)
\end{definition}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\begin{example}
Suposem que volem estudiar \textbf{3 característiques físiques} de \textbf{5 persones}:
...
\tightlist
\item
  cada \textbf{fila} representa una persona;
\item
  les \textbf{files} es consideren independents entre si;
\item
  les \textbf{columnes} poden estar correlacionades. Per exemple, persones més altes acostumen a tenir també un pes més elevat. Aquesta dependència queda descrita per la matriu de covariàncies \(\boldsymbol{\Sigma}\).
\end{itemize}
...
és a dir, \(S\) segueix una distribució de \textbf{Wishart} amb \(5\) graus de llibertat i matriu de covariàncies \(\boldsymbol{\Sigma}\).
\end{example}
...
\subsubsection{Distribució Dirichlet}\label{distribuciuxf3-dirichlet}
...
\begin{definition}
\textbf{Distribució de Dirichlet}
...
Els moments de la distribució de Dirichlet són els següents. Per simplificar les expressions utilitzarem \{\alpha\emph{0\sum}\{i=1\}\^{}k\alpha\_i\}
...
\subsubsection{Revisió de mètodes de generació de mostres de variables aleatòries univariants}\label{revisiuxf3-de-muxe8todes-de-generaciuxf3-de-mostres-de-variables-aleatuxf2ries-univariants}
...
\textbf{Mètode de la acceptació-rebuig}
...
\textbf{Mètodes de transformacions específics}

\emph{Mètode de Box-Muller}
...
\emph{Transformació Gamma com a suma d'exponencials (convolució) }
...
La idea bàsica es que si \{X\_1,\ldots,X\_N\} son variables i.i.d llavors
\[
E[h(x)] \approx \dfrac {1}{N} \sum_{i=1}^{N}h(X_i)
\]

\subsubsection{Generació de variables aleatòries multivariants utilitzant distribucions condicionals}\label{generaciuxf3-de-variables-aleatuxf2ries-multivariants-utilitzant-distribucions-condicionals}
...
\begin{example}
\protect\hypertarget{exm:simnormbiv}{}\label{exm:simnormbiv}\textbf{Normal Bivariant}
...
\item
  ...
  \end{itemize}
\item
  \textbf{Pas 2 (Generar \(x_2 \mid X_1 = x_1\)):}
...
\pandocbounded{\includegraphics[keepaspectratio]{Probabilitat-II_files/images/simulacio-bivariant-1.pdf}}
\end{example}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{Generació de vectors aleatoris de una distribució normal multivariant}\label{generaciuxf3-de-vectors-aleatoris-de-una-distribuciuxf3-normal-multivariant}
...
\textbf{Descomposició de Cholesky de una matriu simètrica definida positiva}
...
\textbf{Simulació d'una Normal Bivariant mitjançant la Descomposició de Cholesky}
...
\emph{(Observem que s'obté exactament la mateixa relació que en el mètode de les condicionals).}
...
\pandocbounded{\includegraphics[keepaspectratio]{Probabilitat-II_files/images/simulacio-cholesky-1.pdf}}
...
\(T\) es un subconjunt de \(\mathbb{R}\) que s'anomena \textbf{conjunt d'indexs}. S'utilitza la lletra \(t\) donat que l'estudi de processos estocàstics surt del estudi de l'evolutcó temporal de fenòmens aleatoris.

El conjunt de valors \(S \subseteq \mathbb{R}\) que poden prendre les variables aleatòries \(X_t\) s'anomena \textbf{espai d'estats} del procés.
...
\begin{example}
\protect\hypertarget{exm:cadena}{}\label{exm:cadena}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:graf-pbernoulli} Exemple Compra botiga de roba}]{Probabilitat-II_files/images/graf-pbernoulli-1.pdf}}
\caption{\label{fig:graf-pbernoulli} Exemple Compra botiga de roba}
\end{figure}
...
Un jugador comença amb \textbf{10 euros} i juga consecutivament \textbf{20 partides}. En cada partida aposta 1 euro a cara o creu.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:unnamed-chunk-1}Exemple de procés del jugador arruïnat}]{Probabilitat-II_files/images/unnamed-chunk-1-1.pdf}}
\caption{\label{fig:unnamed-chunk-1}Exemple de procés del jugador arruïnat}
\end{figure}

\end{example}

\begin{example}
\protect\hypertarget{exm:ppuntual}{}\label{exm:ppuntual}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:graf-ppoisson}Exemple de nombre d'urgències per temps}]{Probabilitat-II_files/images/graf-ppoisson-1.pdf}}
\caption{\label{fig:graf-ppoisson}Exemple de nombre d'urgències per temps}
\end{figure}

\end{example}

\begin{example}
\protect\hypertarget{exm:pcontinu}{}\label{exm:pcontinu}En una prova d'esforç es mesura el consum d'oxigen d'un esportista (\(VO_2\)) cada segon durant 10 minuts i es recull en un procés estocàstic \(X(t)=\text{valor del } V0_2 \text{ en l'instant }t\)
...
La línia blava representa una possible realització del procés corresponent a les mesures de \$VO\_2. La línia vermella discontínua representa la tendència mitjana esperada durant l'exercici. En aquest cas es representa una sèrie temporal biomèdica on clarament les observacions no son independents
\end{example}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\label{fig:graf-pgaussia}Exemple de nombre d'urgències per temps}]{Probabilitat-II_files/images/graf-pgaussia-1.pdf}}
\caption{\label{fig:graf-pgaussia}Exemple de nombre d'urgències per temps}
\end{figure}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

Un procés estocàstic també pot ser considerat com una funció aleatòria de doble argument \(\{ X(t,\omega), t \in T , \omega \in \Omega\}\). Si fixem \(\omega=\omega_0\) tindrem una \emph{realització} del procés \(X(.,\omega_0)\) on la seva representació gràfica son le anomenades \emph{trajectòries del procés} que hem representant en els 4 gràfics anteriors.
...
\subsubsection{Funcions de moment d'un procés estocàstic}\label{funcions-de-moment-dun-procuxe9s-estocuxe0stic}
...
\textbf{Funció mitjana d'un procés estocàstic}
...
\end{cases}   \label{eq:meanprocess}
\]

\textbf{Funció de autocorrelació}
...
& \text{si } X_t \text{ és continu}.
\end{cases} \label{eq:acfprocess}
\]

\textbf{Funció de autocovariància}

A partir del moment central conjunt de dues variables associades a dos temps qualsevol, \(t_1\) i \(t_2\), l'autocovariància és

\[
  C(t_1,t_2)=E[(X_{t_1}-\mu(t1))(X_{t_2}-\mu(t2))] \label{eq:aujtocovprocess}
\]
...
en el cas particular que \(t_1=t_2=t\) parelm de la \emph{funció de variància} \(\sigma^2(t)=C(t,t)\)
...
Els processos estocàstics es poden classificar en funció del tipo de variables T i X\_t

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}ccc@{}}
\toprule\noalign{}
\textbf{\(X_t\) } & \textbf{T Discret} & \textbf{T Continu} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{Discret} & Cadena & Proc. puntual \\
\textbf{Continu} & Suc. de v.a. & Proc. continu \\
\end{longtable}
}
...
-Un procés continu en un temps discret (\textbf{DTCV}- Discret Time/Continous Values)s'anomena un successió de variables. Si en el cas anterior en lloc de mesurar si compra o no compra haguéssim mesurat l 'alçada o el pes del client seria un bon exemple

-Un procés puntual com el de l'exemple \ref{exm:ppuntual} on es mesura el nombre de urgències en un determinat temps és el cas on el procés es discret pero el temps és continu.(\textbf{CTDV}+-Continous Time/Discret Values)

-El procés continu es aquell en el que la variable es continua i el temps també(\textbf{CTCV}+-Continous Time/Continous Values) . Es el cas de l'exemple \ref{exm:pcontinu} on es mesura el \(VO_2\) d'un esportista en 10 minuts d'esforç
...
\begin{definition}
\textbf{Distribucions finito-dimensionals}
...
\begin{definition}
\textbf{Processos amb increments independents}
...
\begin{definition}
\textbf{Processos estacionaris forts}
...
\begin{definition}
\textbf{Processos estacionaris dèbil}
...
\textbf{Probabilitats inicials}

Les probabilitats inicials són les probabilitats de cada un dels possibles estat a l'inici del procés

\[ 
\pi_i=P(X_0=s_i), \quad  \pi_i \geq 0, \forall i=1,...,k, \qquad \Sigma_i \pi_i=1  \label{eq:pi}
\]
\textbf{Probabilitats de transició}
...
\[
 \pi_{ij}=P(X_n+1=s_j \mid X_n+1=s_i )  \label{eq:pij} 
 \]

Si el valor de la probabilitat de transició \(\pi_{ij}\) no depen del tems, es a dir la probabilitat de passar d'un estat a un altre només depen de l'estat i no del temps, tenim probabilitats de \textbf{transició estacionaries ó homogènies}.
...
\begin{example}
\protect\hypertarget{exm:climapoble}{}\label{exm:climapoble}Imaginem que ens anem de vacances al poble i suposem que el clima del poble pot estar en un d'aquets tres estats possibles Sol(S), Nuvolat(N) ó Pluja(P)
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/graf-clima-1} \caption{Exemple de cadena de Markov del clima al poble}\label{fig:graf-clima}
\end{figure}
...
\textbf{Probabilitats de transició a n passos}
...
\[
\textbf{P}(n)=\textbf{P}(n-1) \textbf{P}=\textbf{P}^n   \label{eq:ptranspn}
\]

En general \(\textbf{P}(n+m)=\textbf{P}^n\textbf{P}^m\) que terme a terme ho expresem com
\[
p_{ij}(n+m)=\sum_k p_{ik}(n)p_{kj}(m), \qquad n,m>0, \forall (s_i,s_j)   \in S   \label{eq:chapman}
\]
Aquestes expressions son conegudes com les equacions de \textbf{Chapman-Kolmogorov}

\textbf{Probabilitats dels estats desprès de n passos}
...
\tightlist
\item
  \textbf{Probabilitat de Sol d'ací a 5 dies:} La probabilitat de tenir un dia assolellat és del \textbf{45.17\%}(un valor de 0.4517).
\item
  \textbf{Probabilitat de Pluja d'ací a 5 dies:} La probabilitat que plogui d'ací a cinc dies és del \textbf{19.35\%} (un valor de 0.1935).
\end{itemize}
...
\begin{example}
\protect\hypertarget{exm:dequilibri}{}\label{exm:dequilibri}Si \(X_n\) és la variable que indica si el n-ésim client en una botiga compra o no compra, es tracta d'una cadena de Markov discreta amb espai d'estats \(S = \{0=\text{no compra}, 1=\text{compra} \}\) i matriu de transició:
...
\begin{itemize}
\item
  \(\alpha\): probabilitat de passar de \emph{No Compra} a \emph{Compra}.
\item
  \(\beta\): probabilitat de passar de \emph{Compra} a \emph{No Compra}.
\end{itemize}
...
la qual \textbf{no depèn de la distribució inicial}.
\end{example}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\begin{definition}
\textbf{Equilibri d'una cadena de Markov}
...
\]
Així la distribució sobre l'espai d'estats ten a una distribució fixa independent de la distribució inicial \(\boldsymbol{\pi}\) Es diu que la cadena ha arribat a l'equilibri o a una situació estables- La distribució \(\boldsymbol{\pi^{(e)}}\) rep el nom de \emph{distribució d'equilibri o estacionaria}
...
\[
\boldsymbol{\pi^{(e)}}=\boldsymbol{\pi^{(e)}} \boldsymbol{P}   \label{eq:pie} 
\]
...
La distribució estacionària també es pot obtenir utilitzant l'equació \eqref{eq:pie} . Si \(\boldsymbol{\pi}^{(e)}=\left[\pi^{(e)}_0,\,\pi^{(e)}_1\right]\)
...
\subsubsection{Classificació dels estats i comunnicacions d'una cadena de Markov}\label{classificaciuxf3-dels-estats-i-comunnicacions-duna-cadena-de-markov}
...
\begin{definition}
Un \textbf{camí} \(i \rightarrow j\) és una seqüència de transicions entre estats que porta de l'estat \(i\) a l'estat \(j\).
\end{definition}

\begin{definition}
L'estat \(j\) es un \textbf{estat accessible} des d'\(i\) si existeix un camì \(i \rightarrow j\) que porta de \(i\) a \(j\). En aquest cas existeix un \(k\) tal qué \(p_{ij}^{(k)>0}\).
\end{definition}

\begin{definition}
Els estats \(i\) i \(j\) són \textbf{estats comunicats} si existèix un camí de \(i\) a \(j\) i un altre de \(j\) a \(i\). Ho representarem com \(i \leftrightarrow j\)
\end{definition}

\begin{definition}
Definim una \textbf{classe d'estats o de comunicació} es un subconjunt de estats del que cada parell d'estats (considerant el mateix estat) estan comunicats. La coumicació es una relació d'equivalència que indueix una partició en l'espai d'estats.
\end{definition}

\begin{definition}
Es diu que un cadena de Markov és \textbf{irreductible} quant tots els seus estats comuniquen. És a dir \(\forall i, j  \quad \exists k  \text{ tal que } p_{ij}^{(k)}>0\) . En aquest cas només hi ha una classe de comunicació.
\end{definition}
...
\begin{definition}
Un estat \(i\) és \textbf{transitori} si existeix un estat \(j\) i una camí \(i \rightarrow j\) i no existeix cap camí
...
\begin{definition}
Un estat \(i\) que no és transitori s'anomena \textbf{estat recurrent}. En un estat recurrent la probabilitat de (eventualment) regresar a \(i\) es u es a dir \(P(X_n=s_i \mid X_0=s_i)=1\) per algú \(n\geq 1\)
\end{definition}
...
\begin{itemize}
\item
  Els estats \textbf{0} i \textbf{4} són \textbf{absorbents}.
\item
  Els estats \textbf{1}, \textbf{2} i \textbf{3} són \textbf{transitoris}.
\item
  Els estats \textbf{0} i \textbf{4} són \textbf{recurrents}.
\item
  Les parelles d'estats \textbf{(1,2)}, \textbf{(1,3)} i \textbf{(2,3)} es comuniquen.
\end{itemize}
...
\begin{definition}
Una \textbf{classe} A s'anomena \textbf{tancada} si quan es comença en qualsevol estat d'A sempre ens quedem a A. ÉEs a dir
...
\begin{definition}
Direm que l'estat \(i\) és \textbf{periòdic} amb període \(k>1\) si \(k\) és l'enter més petit tal que la longitud de qualsevol camí de \(i \leftarrow i\) es múltiple de k.
...
\begin{definition}
Un estat que no és periòdic s'anomena \textbf{aperiòdic}. Ës aquell estat \(i\) per al que existeix un enter positiu \(r\) tal que \(p_{i,i}^r>0\) i \(p_{i,i}^{r+1}>0\)
\end{definition}

\begin{definition}
Una cadena de Markov es \textbf{ergòdica} si és irreductible i aperiòdica
\end{definition}
...
\subsection{Passeig o Caminades aleatòries.}\label{passeig-o-caminades-aleatuxf2ries.}
...
\begin{definition}
Sigui \(\{Y1,\dots\}\) una successió de variables aleatòries independents idènticament distribuïdes que prenen els valors \(1\) i \(-1\) amb una probabilitat \(p\in [0,1]\) i \(q=1-p\), respectivament. La \textbf{caminada aleatòria (``random walk'')} simple amb paràmetre p es la successió

\[
S_n=S_0+Y_1+Y_2+\dots+Y_n, \quad  n\geq 0  \label{eq:rwalk}
\]
...
\[
S_n=S_{n+1}+ Y_n, \quad n \geq 1  \label{eq:rwalkseq}
\]
\end{definition}
...
\[
\Pr(S_n = k) = \binom{n}{\frac{n+k}{2}} p^{\frac{n+k}{2}} (1-p)^{\frac{n-k}{2}}, \quad k \in \{-n, -n+2, \dots, n-2, n\}.  \label{eq:prwalk}
\]
...
\begin{itemize}
\item
  \emph{Homogeneïtat espacial:} \(P(S_n=k\mid S_0=a)P(Sn=k+b\mid S_0=a+b )\)
\item
  \emph{Homogeneïtat temporal:} \(P(S_n=k\mid S_0=a)P(S_{n+m}=k\mid S_m=a )\)
\item
  \emph{Propietat de Markov:} Per a \(0<i_0<i_1<\dots<i_m<n\) llavors
\end{itemize}
...
Suposem que p=1/2 . En aquest cas es diu que \(S_n\) es una \textbf{caminata aleatòria simple simètrica}. En aquest cas el vector aleatori \(\textbf{Y}^n=\{Y_1\dots,Y_n\}\) es distribueix uniformement en \(\Omega=\{-1,1\}^n\) Axi el nombre de combinacions serà \(2^n\) que sera el número de camins diferents.

I seguint l'equació \eqref{eq:prwalk} la probabilitat de \(S_n\) és:

\[
\Pr(S_n = k) = \binom{n}{\frac{n+k}{2}} \frac{1}{2^n}, \quad k \in \{-n, -n+2, \dots, n-2, n\}.
\]

En la taula i la gràfica gràfica és mostra una caminata aleatòria de longitut \textbf{\(n=200\)} on el valor final es \textbf{\(S_{200}=14\)} i la probabilitat d'obtenir aquest valor és \textbf{\(P(S_{200}=14)=0.0345913\)}
...
\begin{theorem}
\textbf{Recompte de camins que no creuen el 0}
Si \(b>0\) el nombre de camins que porten de \(80,0\) a (n,b) sense tornar a tocar l'eix d'abscisses es \((b/n)N_n(0,b)\)
\end{theorem}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\emph{El problema de les paperetes (Ballot Problem)}

En una votació hi ha dos candidats, \(A\) i \(B\). Suposem que el candidat \(A\) rep \(\alpha\) vots i el candidat \(B\) rep \(\beta\) vots, amb\$ \alpha\textgreater{}\beta.\$ Suposant que les paperetes es compten en un ordre completament aleatori, es vol determinar la probabilitat que durant tot l'escrutini el candidat \(A\) vagi sempre per davant del candidat \(B\).

Aquest és un problema històricament molt conegut, anomenat en anglès \emph{Ballot Problem}, resolt per Joseph Louis Bertrand l'any 1887. Constitueix una aplicació clàssica del principi de reflexió en els passeigs aleatoris.
...
\textbf{La distribució del primer retorn a l'origen}

Suposem P(S\_0=0)=1. Sigui T el moment del primer retorn a 0:
...
\qquad n\in\mathbb{N}^{+}. \label{eq:restorn1} 
\]
\end{theorem}
...
Sumposem k\textgreater0. Pel teorema del recompte de camins que no creuen el zeor, el nombre de camins que van de \((0,0)\) a \((n,k\) és \((k/n)N_n(0,k)\). Cadascun té probabilitat \(p^{(n+k)/2} (1-p)^{(n-k)/2}\). Aleshores la probabilitat demanada és:
...
\begin{theorem}
\emph{(Màxim)}. Suposem \(P(S_0=0)=1\) i \(p=1/2\). La funció de probabilitat del màxim \(M_n\) és per a \(m\geq 1\)

\[
P(M_n=M)=
\begin{cases}
\Pr(S_n=m), & \text{si } (n-m) \text{ és parell},\\
\Pr(S_n=m+1), & \text{si } (n-m) \text{ és senar}. \end{cases}  \label{eq:posmax} 
\]
...
\textbf{L'última visita a l'origen}
...
\]
Observeu que, donat que les visites a 0 només pot passar en instants parells, \(L_{2n}\) preu els valors del conjunt \(\{0,2,\dots,2n\}\). Aquesta variable aleatòria té una distribució coneguda com a \emph{distribució arcsinus discreta}

\begin{theorem}
\emph{Llei de l'arcsinus per l'última visita a l'origen}. suposem \(P(S_0=0)=1\) i \(p=1/2\). Aleshores, per \(k \in \{0,1,\dots,n \}\)

\[
P(L_{2n}=2k)=P(S_{2k}=0)P(S_{2n-2k}=0)=\binom{2k}{k}\binom{2n-2k}{n-k}\dfrac{1}{2^{2n}} \label{eq:arcsinus} 
\]
...
\textbf{Probabilitat de visitar una posició donada}
...
\item
  En el cas del passeig aleatori simple simètric \((p=q=1/2)\) és segur que es visitarà la posició r\textgreater0. També ho és si \(p>q\).
\end{itemize}
...
\caption{Representació gràfica de les funcions relacionades amb un procés de Poisson}\label{fig:unnamed-chunk-10}
\end{figure}
...
\[
 g_n(t) =\lambda \dfrac {(\lambda t)^{n-1}} {(n-1)!}e^{-\lambda t},\quad t \geq 0  \label{eq:denserlang} 
 \]
i la funció de distribució és
\[
 G_n(t)
 =\sum_{k\geq n} e^{-\lambda t} \dfrac {(\lambda t)^k}{k!}=1-e^{-\lambda t} \sum_{k\geq 0}^{n-1}\dfrac {(\lambda t)^k}{k!} \label{eq:disterlang} 
 \]
...
\[
 \{N_t \geq n \}=\{S_n \geq t \}
 \]
i combinat l'equació anterior i \eqref{eq:disterlang} tenim
...
*\textbf{Increments Independents i Estacionaris}
...
\tightlist
\item
  Per la \textbf{propietat de falta de memòria}, el temps restant \(Y_{N_t+1}\) manté la mateixa distribució: \(Y_{N_t+1} \sim \text{Exp}(\lambda)\).
\item
...
\textbf{Mitjana del procés:}
Com ja hem vist anteriorment:
\[\mu(t) = \lambda t\]

\textbf{Funció d'autocorrelació:}
...
\emph{Distribució condicional de les arribades:}
...
\emph{Conclusió:} Condicionat a haver-hi \(n\) esdeveniments en \((0, t]\), aquests es distribueixen de forma uniforme i independent al llarg de l'interval.

\textbf{Simulació d'un Procés de Poisson}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{simular\_poisson }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(lambda, t) \{}
  \CommentTok{\# 1. Nombre de successos}
\NormalTok{  n0 }\OtherTok{\textless{}{-}} \FunctionTok{rpois}\NormalTok{(}\DecValTok{1}\NormalTok{, lambda }\SpecialCharTok{*}\NormalTok{ t)}
  \CommentTok{\# 2. Temps d\textquotesingle{}arribada uniformes}
  \ControlFlowTok{if}\NormalTok{ (n0 }\SpecialCharTok{\textgreater{}} \DecValTok{0}\NormalTok{) \{}
\NormalTok{    S }\OtherTok{\textless{}{-}} \FunctionTok{sort}\NormalTok{(}\FunctionTok{runif}\NormalTok{(n0, }\AttributeTok{min =} \DecValTok{0}\NormalTok{, }\AttributeTok{max =}\NormalTok{ t))}
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
\NormalTok{    S }\OtherTok{\textless{}{-}} \FunctionTok{numeric}\NormalTok{(}\DecValTok{0}\NormalTok{)}
\NormalTok{  \}}
  
  \FunctionTok{return}\NormalTok{(}\FunctionTok{list}\NormalTok{(}\AttributeTok{n\_events =}\NormalTok{ n0, }\AttributeTok{temps\_arribada =}\NormalTok{ S))}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
La caminada aleatòria (\emph{random walk}) presentada en un apartat anterior era la suma de desplaçaments independents a dreta i a esquerra, de forma que només estaven permesos salts de un sentit a un altre en cada interval de temps. Imaginem ara despla
...
\tightlist
\item
  \emph{Independència:} \(W(t+\Delta t)- W(t)\) es independent de \$\{ W(\tau) \} \quad  \forall \tau \leq t
\item
  \emph{Estacionarietat:} La distribució de \(W(t+\Delta t)-W(t)\) no depen de t
\item
  \emph{Continuïtat:} \(lim_{\Delta t \rightarrow 0} \dfrac {P(|W(t+\Delta t)- W(t)| \geq \delta)}{\Delta t}=0 \quad \forall \delta >0\)
\item
...
\emph{Propietats del moviment Brownià}
...
\textbf{Fluctuacions del cabal d'un riu}
...
\pandocbounded{\includegraphics[keepaspectratio]{Probabilitat-II_files/images/brownian-river-1.pdf}}
